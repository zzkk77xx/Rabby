...
export class WalletController extends BaseController {
  ...
  getDefiInteractorSafe = () => preferenceService.getDefiInteractorSafe();
  ...
  fetchDefiInteractorSafe = async (
    ...
    // ABI for avatar public variable getter (auto-generated by Solidity)
    // Zodiac Module base contract has: address public avatar;
    const abi = ['function avatar() view returns (address)'];
    ...
    // All chains failed
    console.error('Failed to fetch Safe address from module on all chains');
    return null;
  };
  ...
  fetchGnosisChainList = async (address: string) => {
    ...
    const results: (Chain | null)[] = [];
    const allChains = ensureChainListValid(GNOSIS_SUPPORT_CHAINS);

    // Check Sepolia first (prioritized)
    const sepoliaChain = await checkChain(CHAINS_ENUM.SETH);
    results.push(sepoliaChain);

    // Check Ethereum mainnet second (fallback priority)
    const ethChain = await checkChain(CHAINS_ENUM.ETH);
    results.push(ethChain);

    // Check all other chains in parallel
    const otherChains = allChains.filter(
      (chain) => chain !== CHAINS_ENUM.SETH && chain !== CHAINS_ENUM.ETH
    );
    const otherResults = await Promise.all(otherChains.map(checkChain));
    results.push(...otherResults);

    return results.filter((chain): chain is Chain => !!chain);
  };
