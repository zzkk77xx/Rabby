...
export class WalletController extends BaseController {
  ...
  fetchDefiInteractorSafe = async (moduleAddress: string): Promise<string | null> => {
    try {
      ...
      // Use the current chain or default to ETH
      const currentChain = permissionService.getConnectedSite(INTERNAL_REQUEST_ORIGIN)?.chain || CHAINS_ENUM.ETH;
      const chain = findChain({ enum: currentChain });
      if (!chain) {
        throw new Error('Current chain not found');
      }

      // Make the call using the RPC provider
      const provider = new ethers.providers.Web3Provider(
        buildinProvider.currentProvider as any
      );
      ...
      console.error('Error fetching Safe address from module:', error);
      return null;
    }
  };
